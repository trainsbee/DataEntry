<?php
$users = [
    "MGR001" => [
        "id" => "00186",
        "name" => "DAVID GONZALES MALDONADO",
        "username" => "dmaldonado",
        "password" => "dmaldonado1",
        "DEPARTMENT" => "DATA ENTRY",
        "status" => "active",
        "role" => "admin",
        "employees" => [
            [ "id" => "04394", "name" => "EVELYN DANIELA OSEGUERA AGUILAR", "username" => "eaguilar", "password" => "eaguilar1", "status" => "active", "role" => "user" ],
            [ "id" => "10070", "name" => "ASTRID MARIELA COLINDRES ZELAYA", "username" => "acolindres", "password" => "acolindres1", "status" => "active", "role" => "user" ],
            [ "id" => "10071", "name" => "ERICKA DANIELA MARTINEZ", "username" => "emartinez", "password" => "emartinez1", "status" => "active", "role" => "user" ],
            [ "id" => "04866", "name" => "JESUS ALCIDES SAGASTUME MARTINEZ", "username" => "jsagastume", "password" => "jsagastume1", "status" => "active", "role" => "user" ],
            [ "id" => "22056", "name" => "ANGEL DAVID ALVARENGA MARTINEZ", "username" => "amartinez", "password" => "amartinez1", "status" => "active", "role" => "user" ],
            [ "id" => "21037", "name" => "LOURDES RAQUEL AGUIRRE", "username" => "laguirre", "password" => "laguirre1", "status" => "active", "role" => "user" ],
            [ "id" => "13174", "name" => "AMSI ABIGAIL URQUIA DURON", "username" => "aduron", "password" => "aduron1", "status" => "active", "role" => "user" ],
            [ "id" => "15164", "name" => "DANIEL VLADIMIR HERNANDEZ SALGADO", "username" => "dsalgado", "password" => "dsalgado1", "status" => "active", "role" => "user" ]
        ]
    ],
    "MGR002" => [
        "id" => "00200",
        "name" => "JORGE LUIS RODRIGUEZ MACEDO",
        "username" => "jmacedo",
        "password" => "jmacedo1",
        "DEPARTMENT" => "COMPLIANCE",
        "status" => "active",
        "role" => "admin",
        "employees" => [
            [ "id" => "20638", "name" => "HELEN DANIELA MATUTE ZAMBRANO", "username" => "hzambrano", "password" => "hzambrano1", "status" => "active", "role" => "user" ],
            [ "id" => "15616", "name" => "EMELY RACHEL ROMERO CABRERA", "username" => "ecabrera", "password" => "ecabrera1", "status" => "active", "role" => "user" ],
            [ "id" => "21145", "name" => "YORLENY PAVETH OLIVA MALDONADO", "username" => "ymaldonado", "password" => "ymaldonado1", "status" => "active", "role" => "user" ],
            [ "id" => "22694", "name" => "ISIS AMALETH ARDON MARADIAGA", "username" => "imaradiaga", "password" => "imaradiaga1", "status" => "active", "role" => "user" ],
            [ "id" => "10616", "name" => "ROBERTO CARLOS MERAZ CANALES", "username" => "rcanales", "password" => "rcanales1", "status" => "active", "role" => "user" ],
            [ "id" => "12309", "name" => "FABIOLA YADIRA CASTILLO MONCADA", "username" => "fmoncada", "password" => "fmoncada1", "status" => "active", "role" => "user" ],
            [ "id" => "02936", "name" => "VICTOR ENRIQUE MALDONADO ZELAYA", "username" => "vzelaya", "password" => "vzelaya1", "status" => "active", "role" => "user" ],
            [ "id" => "00543", "name" => "KATERIN ELIZABETH HERNANDEZ MARTINEZ", "username" => "kmartinez", "password" => "kmartinez1", "status" => "active", "role" => "user" ]
        ]
    ],
    "MGR003" => [
        "id" => "03487",
        "name" => "KENIA ISABEL ZAMBRANO MOLINA",
        "username" => "kmolina",
        "password" => "kmolina1",
        "DEPARTMENT" => "CUSTOMER SERVICE",
        "status" => "active",
        "role" => "admin",
        "employees" => [
            [ "id" => "20200", "name" => "WOLFANG ANDRE SOSA LANZA", "username" => "wlanza", "password" => "wlanza1", "status" => "active", "role" => "user" ],
            [ "id" => "00439", "name" => "JONNY OMAR SALINAS ROSALES", "username" => "jrosales", "password" => "jrosales1", "status" => "active", "role" => "user" ],
            [ "id" => "09404", "name" => "DAVID ALEJANDRO SANTOS ORDONEZ", "username" => "dordonez", "password" => "dordonez1", "status" => "active", "role" => "user" ],
            [ "id" => "15245", "name" => "MARCELO ALEJANDRO FOLGAR BONILLA", "username" => "mbonilla", "password" => "mbonilla1", "status" => "active", "role" => "user" ],
            [ "id" => "10716", "name" => "ALEJANDRO JOSE SALGADO GIRON", "username" => "agiron", "password" => "agiron1", "status" => "active", "role" => "user" ]
        ]
    ],
    "MGR004" => [
    "id" => "00731",
    "name" => "JORGE ARTURO GOMEZ COELLO",
    "username" => "jcoello",
    "password" => "jcoello1",
    "DEPARTMENT" => "NEGOTIATION AND INSURANCE ",
    "status" => "active",
    "role" => "admin",
    "employees" => [
        [ "id" => "14537", "name" => "LUIS FRANCISCO MURILLO CARCAMO", "username" => "lcarcamo", "password" => "lcarcamo1", "status" => "active", "role" => "user" ],
        [ "id" => "02150", "name" => "CLAUDETTE DESIREE ROLLINGS MARTINEZ", "username" => "cmartinez", "password" => "cmartinez1", "status" => "active", "role" => "user" ],
        [ "id" => "05878", "name" => "VLADIMIR RODRIGUEZ CASTEJÓN", "username" => "vrcastejon", "password" => "vrcastejon1", "status" => "active", "role" => "user" ],
        [ "id" => "02088", "name" => "ENRIQUE ALEJANDRO CASCO MURILLO", "username" => "emurillo", "password" => "emurillo1", "status" => "active", "role" => "user" ],
        [ "id" => "09157", "name" => "GERARDO LEONEL FERNANDEZ MARTINEZ", "username" => "gfernandez", "password" => "gfernandez1", "status" => "active", "role" => "user" ],
        [ "id" => "14588", "name" => "SETH ISAÍ GODOY CANTARERO", "username" => "sgodoy", "password" => "sgodoy1", "status" => "active", "role" => "user" ],
        [ "id" => "17862", "name" => "RAÚL EDUARDO ESPINOZA NÚÑEZ", "username" => "respinoza", "password" => "respinoza1", "status" => "active", "role" => "user" ],
        [ "id" => "02339", "name" => "WALTER ERNESTO VARELA SEVILLA", "username" => "wvarela", "password" => "wvarela1", "status" => "active", "role" => "user" ]
        ]
],
"MGR005" => [
            "id" => "34210",
            "name" => "MANAGER SUPER ADMINISTRADOR",
            "username" => "superadmin",
            "password" => "superadmin1",
            "status" => "active",
            "role" => "superadmin",
            "employees" => [
                [ "id" => "1", "name" => "TEST 1", "username" => "test1", "password" => "test1", "status" => "active", "role" => "user" ],
                [ "id" => "2", "name" => "TEST 2", "username" => "test2", "password" => "test2", "status" => "active", "role" => "user" ],
                [ "id" => "3", "name" => "TEST 3", "username" => "test3", "password" => "test3", "status" => "active", "role" => "user" ],
            ]
        ],  
        "MGR006" => [
            "id" => "00142",
            "name" => "FRANCISCO JAVIER MENDEZ MONTENEGRO",
            "username" => "fmontenegro",
            "password" => "fmontenegro1",
            "status" => "active",
            "role" => "admin",
            "employees" => [
                [ "id" => "1", "name" => "TEST 1", "username" => "test1", "password" => "test1", "status" => "active", "role" => "user" ],
                [ "id" => "2", "name" => "TEST 2", "username" => "test2", "password" => "test2", "status" => "active", "role" => "user" ],
                [ "id" => "3", "name" => "TEST 3", "username" => "test3", "password" => "test3", "status" => "active", "role" => "user" ],
            ]
        ],
];
?>